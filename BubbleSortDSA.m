...
(*AppendFrame[frames_, newStep_, newComparisons_, newFinal_] := Module[{fr = frames, step = newStep, comp = newComparisons, fin = newFinal, *)
(*																	steps, comparisons, final},*)
...
(*BubbleSortFrames[list_, optimized_]:=Module[{l=list, opt = optimized, i,j,len=Length[list], n, frames, final},*)
...
(*									       l = Swap[l, j, j+1];*)
(*                                                                                       (*update frames*)*)
(*								frames = AppendFrame[frames, l, Last[Part[frames,2]], final],*)
(*									      True],*)
(*									  {j,1,n-1}]; *)
(**)
(*								(*if optimized bubblesort, add final elements and consider one less element to compare*)*)
(*								 If[opt, frames = AppendFrame[frames, l, {}, Join[Last[Part[frames,3]], List[n]]];*)
(*									 n = n-1, True],								      *)
(*								{i,1,len-1}]; *)
(*								AppendFrame[frames, l, {}, Range[1,len]]]*)
(**)
(*(*Function to create the bubblesort animation*)*)
(*BubbleSortAnimation[length_, toggler_, optimized_:False] := Module[{len = length, opt = optimized, array, frames, steps, comparisons, final},*)
...
(*BubbleSortGame[length_, optimized_:False] := DynamicModule[{len = length, opt = optimized,originalArray, steps, *)
(*												currentArray, arrayDisplay, trial, swapNumber, enabled, i,j},*)
